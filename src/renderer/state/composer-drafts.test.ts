...
import { composerDraftHasContent, composerDraftOwnerKey, journalComposerDraftSend } from './composer-drafts'
...
describe('composer draft state', () => {
  it('uses a thread-stable owner after a session exists', () => {
    expect(composerDraftOwnerKey('project one', 'window one')).toBe('project%20one/window%3Awindow%20one')
    expect(composerDraftOwnerKey('project one', 'window one', 'thread one')).toBe('project%20one/thread%3Athread%20one')
  })
