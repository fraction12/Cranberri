...
export function AgentStatusIcon({ status, stopping = false }: { status: CodexWorkerStatus; stopping?: boolean }) {
  if (stopping) return <Loader2 className="h-3.5 w-3.5 animate-spin text-app-status-warning" aria-hidden="true" />
  if (codexWorkerIsActive(status)) return <Loader2 className="h-3.5 w-3.5 animate-spin text-app-accent" aria-hidden="true" />
  if (status === 'completed') return <CheckCircle2 className="h-3.5 w-3.5 text-app-status-success" aria-hidden="true" />
  if (status === 'errored' || status === 'notFound') return <AlertCircle className="h-3.5 w-3.5 text-app-status-danger" aria-hidden="true" />
  return <Power className="h-3.5 w-3.5 text-app-text-secondary" aria-hidden="true" />
}
