...
export function TaskHeader({ task, branch, onOpen, onOpenTerminal, onHandoff, onRetrySetup, onArchive, onUnarchive }: TaskHeaderProps) {
  ...
  return (
    <header className="flex h-9 shrink-0 items-center gap-2 px-3">
      ...
      {onOpen && <IconButton type="button" label="Open task folder" onClick={onOpen}><FolderOpen className="h-3.5 w-3.5" /></IconButton>}
      <DropdownMenu.Root>
        <DropdownMenu.Trigger asChild><IconButton type="button" label="Task actions"><Ellipsis className="h-3.5 w-3.5" /></IconButton></DropdownMenu.Trigger>
        <DropdownMenu.Portal>
