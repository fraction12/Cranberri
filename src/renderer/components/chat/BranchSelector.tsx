...
export function BranchSelector({ value, options, loading = false, partialFallback = false, includeLocalEligible = false, includeLocalChanges = false, onChange, onIncludeLocalChanges, onRetry }: {
  value: string; options: readonly BranchOption[]; loading?: boolean; partialFallback?: boolean; includeLocalEligible?: boolean; includeLocalChanges?: boolean; onChange: (ref: string) => void; onIncludeLocalChanges?: (include: boolean) => void; onRetry?: () => void
}) {
  ...
  return <DropdownMenu.Root><DropdownMenu.Trigger asChild><button type="button" aria-label={`Base branch: ${selected?.label ?? value}`} title={selected?.label ?? value} className={triggerClass}><GitBranch className="h-3.5 w-3.5" /><span className="max-w-32 truncate">{selected?.label ?? value}</span><ChevronDown className="h-3 w-3 text-app-text-muted" /></button></DropdownMenu.Trigger><DropdownMenu.Portal><DropdownMenu.Content side="top" align="start" sideOffset={7} collisionPadding={10} className={cn(menuSurface, 'z-[1300] w-64 max-h-[min(340px,calc(100vh-32px))] overflow-y-auto overscroll-contain')} onWheel={(event) => event.stopPropagation()}>
