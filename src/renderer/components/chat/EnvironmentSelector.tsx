...
export function EnvironmentSelector({ value, options, defaultId, onChange }: { value: string | null; options: readonly EnvironmentOption[]; defaultId?: string | null; onChange: (id: string | null) => void }) {
  ...
  return <DropdownMenu.Root><DropdownMenu.Trigger asChild><button type="button" aria-label={`Environment: ${selected?.name ?? 'No environment'}`} title="Environment" className={triggerClass}><PackageOpen className="h-3.5 w-3.5" /><span className="max-w-36 truncate">{selected?.name ?? 'No environment'}</span><ChevronDown className="h-3 w-3 text-app-text-muted" /></button></DropdownMenu.Trigger><DropdownMenu.Portal><DropdownMenu.Content side="top" align="start" sideOffset={7} collisionPadding={10} className={cn(menuSurface, 'z-[1300] w-60 max-h-[min(320px,calc(100vh-32px))] overflow-y-auto overscroll-contain')} onWheel={(event) => event.stopPropagation()}>
