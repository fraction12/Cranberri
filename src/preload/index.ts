...
const api = {
  ...
  tasks: {
    ...
    createLocalDraft: (request: import('@/shared/tasks').LocalTaskDraftRequest) => ipcRenderer.invoke('tasks:create-local-draft', request),
    adoptLocalThread: (request: import('@/shared/tasks').LocalTaskAdoptRequest) => ipcRenderer.invoke('tasks:adopt-local-thread', request),
    ...
    continueInWorktree: (taskId: string) => ipcRenderer.invoke('tasks:continue-in-worktree', { taskId }),
